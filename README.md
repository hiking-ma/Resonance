...
「组合回测」中的仓位是历史策略推演，不代表用户实际持仓。「我的仓位」使用独立账本：

- 首次以指定日期、0% 仓位初始化，只响应该日期之后的新策略信号。
- 历史 BUY 不追买；空仓时遇到历史持仓的 SELL 会忽略。
- 次日计划发出后不会自动改变仓位，必须在页面点击「已执行」确认。
- 未执行的计划可「跳过」；确认前系统不会假设已经成交。
- 初次买入目标 12.5%，最早下一交易日才可计划加仓至 25%。
...
交易日 14:30 发送一次多指标共振盘中快照。实时价格、份额与综合信号参与计算；当日成交额
和融资数据尚未完成时记为灰灯，不沿用昨日值参与共振计数。无共振也会发送任务正常回执。

交易日 22:30 系统根据新策略信号和「我的仓位」中已确认的实际仓位，生成次交易日操作计划。
有操作时发送买入、加仓、减仓或清仓计划，无操作也发送回执；数据未齐或存在待确认计划时
发送明确提示。执行后需到「我的仓位」手动确认。
